...
Persist best score in `localStorage` under `gs_best:<slug>`. **Derive `<slug>` from your URL at runtime** — e.g. `const SLUG = (location.pathname.match(/\/play\/([a-z0-9-]+)/)||[])[1] || 'game';` — rather than hardcoding it, so that a REMIX (a copy served at a new slug) keeps its own saved data instead of colliding with the original's. Persist any progression save under `gs_save:<slug>` the same way. (The platform also rewrites the old slug when it copies a game for remix, and its "start over" control wipes every localStorage key containing the game's slug — but deriving the slug from the URL is the robust way to stay remix-safe.) (Optionally also handle a `{gs:'reset'}` postMessage to reset in-memory state for a separate game origin.)
...
4. **Touch works fully — and is SELF-EVIDENT.** Pointer events (not click-only), thumb-zone-aware layout. On a coarse pointer (`matchMedia('(pointer: coarse)')`), **every action available on desktop must have a VISIBLE, LABELED on-screen control** — a drawn button, joystick, slider, or clearly-outlined zone with a label. A first-time player must SEE how to do each thing without being told. **Invisible drag zones are a fail**: never gate a continuous/modal control (pitch, tilt, throttle, steer, aim-hold) behind an unlabeled region or a hint that fades away — draw the control and keep it on screen while it's usable. The single best touch mapping is *direct*: if the verb is "aim/point/place", the finger position IS the input (no proxy). Reserve drag-zones only for genuinely analog inputs, and then draw the zone with a visible handle. Test mentally at 390×844 portrait and 844×390 landscape.
